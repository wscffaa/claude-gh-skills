...
def build_gemini_args(args) -> list:
    ...
    使用位置参数传递 prompt（支持 @filepath 语法引用图像等文件）
    注意：-p/--prompt 已废弃，将在未来版本移除
    """
    return [
        'gemini',
        '-m', DEFAULT_MODEL,
        '--output-format', 'text',
        args['prompt']  # 位置参数，支持 @filepath 多模态引用
    ]
...
def main():
    ...
    try:
        ...
        # 读取 stderr，过滤 Gemini CLI 的噪音日志
        stderr_output = process.stderr.read()
        if stderr_output:
            # 过滤掉 [STARTUP]、[ERROR] [IDEClient] 等无关日志
            filtered_lines = []
            for line in stderr_output.splitlines():
                if any(noise in line for noise in ['[STARTUP]', '[IDEClient]', 'StartupProfiler']):
                    continue
