...
用法:
    ...
    python3 create_project.py --title "名称" --user [--json]  # 用户级 Project
...
功能:
    ...
    3. 默认将 Project 关联到当前仓库（显示在仓库 Projects 页）
...
def link_project_to_repository(project_id: str, repository_id: str) -> None:
    """将 Project 关联到仓库，使其显示在仓库的 Projects 页面。"""
    mutation = """
    ...
    """
    gh_api_graphql(mutation, {"projectId": project_id, "repositoryId": repository_id})
...
def main():
    ...
    parser.add_argument("--user", action="store_true", help="创建用户级 Project（不关联仓库）")
    parser.add_argument("--json", action="store_true", help="JSON 格式输出")
    args = parser.parse_args()

    # 获取仓库信息（用于默认 owner 和关联仓库）
    repo_info = get_repo_info()
    owner = args.owner or (repo_info.get("owner") if repo_info else None) or get_repo_owner()
    if not owner:
    ...
    # 默认关联到当前仓库（除非指定 --user）
    if not args.user and repo_info and repo_info.get("id"):
        link_project_to_repository(project["id"], repo_info["id"])
    ...
    if args.json:
        ...
        print(f"- {project['url']}")
        if not args.user and repo_info:
            print(f"- 已关联仓库: {repo_info['owner']}/{repo_info['name']}")
